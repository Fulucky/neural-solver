...
Prefer the `heatsink_inverse_design` MCP tools under `agent`. In remote `main.py` deployment, MCP tools should use `route: "local"` and call the packaged inference code under `AIInverseDesign/infer` and `AIInverseDesign/common`.
